...
namespace SMEAppHouse.Core.HtmlUtil
{
    public static class HtmlUtil
    {
        ...
        public static string Resolve(string val, bool allTrim = false, params string[] otherElementsToClear)
        {
            ...
            {
                otherElementsToClear.ToList().ForEach(e =>
                {
                    val = val.Replace(e, "");
                });
            }
        ...
        /// <returns></returns>
        public static string GetPageDocument(string sourceUrl, ref string extraDataOnError, IWebProxy webProxy = null, int retries = 2)
        {
            ...
            do
            {
                try
                {
                    var uAgent = "Mozilla/5.0 (Windows NT 6.2; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/29.0.1547.2 Safari/537.36";
                                            ...
                                            string defaultValue = "")
        {
            var node = sourceNode
                        .Descendants(element)
                        .FirstOrDefault(d => d.Attributes.Contains(attribute) && d.Attributes[attribute].Value.Contains(valuePartial));
